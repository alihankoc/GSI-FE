...
$activeForms = $apiCaller->sendRequest(array(
    'api_url' => $_ENV['LINK'].'getLaboratoryForms/5',
    'api_method' => 'get',
));
$completedForms = $apiCaller->sendRequest(array(
    'api_url' => $_ENV['LINK'].'getLaboratoryForms/6',
    'api_method' => 'get',
));
$waitingForms = $apiCaller->sendRequest(array(
    'api_url' => $_ENV['LINK'].'getLaboratoryForms/4',
    'api_method' => 'get',
));
?>
<!DOCTYPE html>
    ...
    <div class="card bg-light text-dark">
        <div class="card-header">
            <ul class="nav nav-pills" style="display: inline-flex;" role="tablist">
                <li class="nav-item">
                    <a class="nav-link" href="laboratory_analysis_page.php">
            ...
            <a class="nav-link active btn-info  btn-sm  pull-right" href="new_external_analysis.php"  style="float: right;">
                        + New External Job</a>
        </div>
            ...
            <div class="tab-content">
                <div id="operational" class="container tab-pane active"><br>
                    <ul class="nav nav-tabs" role="tablist">
                        <li class="nav-item">
                            <a id="waitingTabLink" class="nav-link" data-toggle="tab" href="#waiting">Waiting List <span
                                        class="badge badge-secondary"><?php echo $waitingForms['data']->count;
                                    unset($waitingForms['data']->count); ?></span></a>
                        </li>
                        <li class="nav-item active">
                            <a id="activeTabLink" class="nav-link active" data-toggle="tab" href="#active">Active List <span
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped" id="activeTable" style="font-size: 12px;">
                                    ...
                                    <tr>
                                        ...
                                        <th onclick="sortTable(2,'activeTable','date')">Customer Company</th>
                                        <th onclick="sortTable(2,'activeTable','date')">Type</th>
                                        <th>Creator</th>
                                        <th>Receiver</th>
                                        <th>Processes</th>
                                    </tr>
                                    ...
                                    <?php foreach ($waitingForms['data'] as $frm) { ?>
                                        <tr>
                                            ...
                                            <td ><?php echo date("d-m-Y H:i", strtotime($frm->created_at)); ?></td>
                                            <td><?php echo $frm->external_customer_name; ?></td>
                                            <td><?php echo $frm->external_request_type === 'new' ? 'New' : 'Additional'; ?></td>
                                            <td class="activeOfficer"><?php echo urldecode($frm->creator->name . " " . $frm->creator->surname); ?></td>
                                            <td class="activeReceiver"><?php echo urldecode($frm->external_receiver_name); ?></td>
                                            <td>
                                    ...
                                    <?php }
                                    if (empty($waitingForms['data'])) {
                                        ?>
                                        <tr>
                                            <td colspan="11" class="text-center">
                                                <p><strong>There is no waiting form.</strong></p>
                                            </td>
                                        </tr>
                                        <?php
                                    }
                                ...
                                <table class="table table-striped" id="completedTable" style="font-size: 12px;">
                                    ...
                                    <?php foreach ($completedForms['data'] as $frm) { ?>
                                        <tr>
                                            ...
                                            <td class="activeOfficer"><?php echo urldecode($frm->creator->name . " " . $frm->creator->surname); ?></td>
                                            <td class="activeReceiver"><?php echo urldecode($frm->external_receiver_name); ?></td>
                                            <td><?php echo $frm->completer->name . ' '. $frm->completer->surname; ?></td>
                                            <td><?php echo date("d-m-Y H:i", strtotime($frm->completed_date)); ?></td>
                                            <td>
                                                <div class="btn-group dropdown">
                                                    <button type="button" class="btn btn-primary dropdown-toggle btn-sm"
                                                            data-toggle="dropdown" aria-haspopup="true"
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title" id="viewDetailedModalWaitingTitle">
                    Laboratory Form ID: <strong id="waitingID" style="text-transform:capitalize;"></strong><br/>
                    Created At: <strong id="waitingDate" style="text-transform:capitalize;"></strong><br/>
                    Creator: <strong id="waitingCreator" style="text-transform:capitalize;"></strong><br/>
                    Receiver: <strong id="waitingReceiver" style="text-transform:capitalize;"></strong><br/>
                    Customer: <strong id="waitingCustomer" style="text-transform:capitalize;"></strong><br/>
                </h6>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
...
<div class="modal fade" id="viewDetailedModalActive" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title" id="viewDetailedModalWaitingTitle">
                    Laboratory Form ID: <strong id="activeID" style="text-transform:capitalize;"></strong><br/>
                    Created At: <strong id="activeDate" style="text-transform:capitalize;"></strong><br/>
                    Creator: <strong id="activeCreator" style="text-transform:capitalize;"></strong><br/>
                    Receiver: <strong id="activeReceiver" style="text-transform:capitalize;"></strong><br/>
                    Customer: <strong id="activeCustomer" style="text-transform:capitalize;"></strong><br/>
                </h6>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('.viewDetailedButtonActive').on('click', function () {
            ...
            var postData = {selectedLabDetW: idOfForm};
            ...
            $.ajax({
                ...
                success: function (resultData) {
                    ...
                    if (result.conditions.length > 0) {
                        ...
                        result.conditions.forEach(function (anlc, index) {

                            if (index === result.conditions.length - 1 || result.conditions.length === 1) {
                                anlcTString += anlc.condition_name;
                            } else {
                                anlcTString += (anlc.condition_name + ", ");
                            }
                    ...
                    const date = new Date(result.lab_form.created_at).toLocaleDateString()
                    $('#activeID').text(result.lab_form.lab_form_id)
                    $('#activeDate').text(date)
                    $('#activeReceiver').text(result.lab_form.external_receiver_name)
                    $('#activeCreator').text(result.lab_form.creator.name + ' ' + result.lab_form.creator.surname)
                    $('#activeCustomer').text(result.lab_form.external_customer_name)
                    if (result.lab_form.analyzes.length > 0) {

                        result.lab_form.analyzes.forEach(function (anl, index) {

                            const doneAnalysis = result.lab_form.done_analysis.find(x => x.analysis_id === anl.analysis_id )
        ...
        $('.viewDetailedButtonWaiting').on('click', function () {
            ...
            var postData = {selectedLabDetW: $(this).prop('id')};
            ...
            $.ajax({
                ...
                success: function (resultData) {
                    ...
                    $('#waitingReceiver').text(result.lab_form.external_receiver_name)
                    $('#waitingCreator').text(result.lab_form.creator.name + ' ' + result.lab_form.creator.surname)
                    $('#waitingCustomer').text(result.lab_form.external_customer_name)
                    const date = new Date(result.lab_form.created_at).toLocaleDateString()
                    $('#waitingID').text(result.lab_form.lab_form_id)
                    $('#waitingDate').text(date)
                    if (result.conditions.length > 0) {
